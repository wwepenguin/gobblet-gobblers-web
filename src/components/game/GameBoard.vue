    ...
    <div class="board-grid">
      ...
      >
        <div 
          ...
          :key="`cell-${x}-${y}`" 
          class="board-cell"
          :class="{ 'cell-highlight': isValidMove(x, y) }"
          @click="handleCellClick(x, y)"
        >
          <div v-if="getTopPiece(cell.stack)" class="piece-container">
            <GamePiece 
              :piece="getTopPiece(cell.stack)" 
              :selectable="isPieceSelectable(getTopPiece(cell.stack))" 
              :selected="isPieceSelected(x, y)"
              @select="selectBoardPiece(getTopPiece(cell.stack), x, y)"
            />
          </div>
